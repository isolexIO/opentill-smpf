...
import { 
  Vault, TrendingUp, Lock, ArrowRightLeft, 
...
export default function DUCVault() {
  ...
  const handleClaim = async () => {
    if (!walletConnected) { alert('Please connect your wallet first'); return; }
    setClaiming(true);
    try {
      const impersonatedUserJSON = localStorage.getItem('pinLoggedInUser');
  ...
  const handleStake = async () => {
    if (!walletConnected) { alert('Please connect your wallet first'); return; }
    if (!stakeAmount || parseFloat(stakeAmount) <= 0) { alert('Please enter a valid stake amount'); return; }
  ...
  const handleSwap = async () => {
    if (!walletConnected) { alert('Please connect your wallet first'); return; }
    if (!swapAmount || parseFloat(swapAmount) <= 0) { alert('Please enter a valid swap amount'); return; }
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            {purchaseChipId && (
              <ChipPurchase 
                ...
                onSuccess={() => { setPurchaseChipId(null); loadVaultData(); }}
              />
            )}
            ...
            {!walletConnected && (
              <Alert className="bg-yellow-50 border-yellow-200">
                ...
                <AlertDescription className="text-yellow-800">
                  ...
                  <Button variant="link" className="text-yellow-600 underline p-0 h-auto ml-2"
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Card>
                ...
                <CardContent><p className="text-sm text-gray-500">$DUC</p></CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-2">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      ...
                      <Input type="number" placeholder={`Max: ${available.toFixed(2)}`}
                        value={claimAmount} onChange={(e) => setClaimAmount(e.target.value)} max={available} />
                    </div>
                    <Button onClick={handleClaim} disabled={claiming || !walletConnected || available <= 0} className="w-full">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input type="number" placeholder="Enter amount" value={stakeAmount}
                        onChange={(e) => setStakeAmount(e.target.value)} />
                    </div>
                    <Button onClick={handleStake} disabled={staking || !walletConnected} className="w-full">
                    ...
                    {stakes.length > 0 && (
                      <div className="mt-6 space-y-2">
                        ...
                        {stakes.map(stake => (
                          <div key={stake.id} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      ...
                      <Input type="number" placeholder="Enter amount" value={swapAmount}
                        onChange={(e) => setSwapAmount(e.target.value)} />
                    </div>
                    <div>
                      <Label>Swap To</Label>
                      <select className="w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
                        value={swapTo} onChange={(e) => setSwapTo(e.target.value)}>
                    ...
                    <Alert>
                      ...
                      <AlertDescription>Swaps are powered by Jupiter with our referral code applied</AlertDescription>
                    </Alert>
                    <Button onClick={handleSwap} disabled={swapping || !walletConnected} className="w-full">
                ...
                {rewardHistory.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {rewardHistory.slice(0, 10).map(reward => (
                      <div key={reward.id} className="flex justify-between items-center p-3 border-b dark:border-gray-700">
                        <div>
