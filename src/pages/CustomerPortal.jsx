...
export default function CustomerPortal() {
    ...
    return (
      <div className="min-h-screen relative flex flex-col items-center justify-center p-6 text-white" style={{ backgroundImage: `url(${PORTAL_BG})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <button onClick={() => setShowQR(false)} className="absolute top-4 left-4 z-10 flex items-center gap-1 text-white/80 hover:text-white">
          <ArrowLeft className="w-5 h-5" /> {t('portal.back')}
        </button>
        <h2 className="relative z-10 text-xl font-bold mb-1">{t('portal.payWithDuc')}</h2>
        <p className="relative z-10 text-sm text-white/70 mb-6">{t('portal.showQr')}</p>
        ...
        <div className="relative z-10 mt-6 text-center">
          <p className="text-sm text-white/70">{t('portal.availableBalance')}</p>
  ...
  if (customer) {
    const stats = [
      { label: t('portal.loyaltyPoints'), value: customer.loyalty_points.toLocaleString(), icon: Coins, color: 'text-amber-600 bg-amber-50' },
      { label: t('portal.ducBalance'), value: customer.duc_balance.toFixed(2), icon: Wallet, color: 'text-purple-600 bg-purple-50' },
      { label: t('customers.totalSpent'), value: `$${customer.total_spent.toFixed(2)}`, icon: TrendingUp, color: 'text-green-600 bg-green-50' },
      { label: t('portal.visits'), value: customer.visit_count.toLocaleString(), icon: Calendar, color: 'text-blue-600 bg-blue-50' },
    ];
      ...
      <div className="min-h-screen bg-gradient-to-b from-[#0A0A12] via-[#0d1b2a] to-[#1a1a2e]">
        <div className="relative text-white px-6 py-6" style={{ backgroundImage: `url(${PORTAL_BG})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
          ...
          <div className="relative z-10 max-w-md mx-auto">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h1 className="text-xl font-bold">{t('portal.myRewards')}</h1>
              ...
              <button onClick={handleSignOut} className="flex items-center gap-1 text-sm text-white/80 hover:text-white">
                <LogOut className="w-4 h-4" /> {t('portal.exit')}
              </button>
            </div>
            <div className="bg-white/15 backdrop-blur rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-white/70">{t('portal.welcomeBack')}</p>
        ...
        <div className="max-w-md mx-auto px-4 py-6 space-y-4">
          ...
          {customer.duc_lifetime_earned > 0 && (
            <Card className="bg-gradient-to-r from-purple-50 to-blue-50 border-purple-200">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-gray-600">{t('portal.lifetimeDuc')}</p>
          ...
          >
            ...
            {customer.duc_balance > 0 ? t('portal.payDuc') : t('portal.noDuc')}
          </Button>
          ...
          <div className="pt-2">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-white">{t('portal.purchaseHistory')}</h3>
            </div>
            {orders.length === 0 ? (
              <Card>
                <CardContent className="p-6 text-center text-sm text-gray-400">
                  {t('portal.noPurchases')}
                </CardContent>
              </Card>
            ) : (
                  ...
                  <Card key={i}>
                    <CardContent className="p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-gray-900">{t('portal.orderLabel')} {order.order_number || `#${i + 1}`}</p>
      ...
      <div className="min-h-screen relative flex items-center justify-center p-6" style={{ backgroundImage: `url(${PORTAL_BG})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="relative max-w-md w-full">
          <div className="text-center text-white mb-8">
            ...
            <h1 className="text-2xl font-bold mb-1">{t('portal.createPin')}</h1>
            <p className="text-sm text-white/70">{t('portal.createPinSub')}</p>
          </div>
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 text-center">
            <p className="text-sm text-blue-800">{t('portal.codeSent')}</p>
            <p className="text-xs text-blue-700 mt-1">{t('portal.codeExpire')}</p>
          </div>
          <Card>
            <CardContent className="p-6">
              <form onSubmit={handlePinSubmit} className="space-y-4">
                <div>
                  <Label htmlFor="vcode">{t('portal.verificationCode')}</Label>
                ...
                <div>
                  <Label htmlFor="pin">{t('portal.newPin')}</Label>
                ...
                <Button type="submit" className="w-full" disabled={authLoading || pin.length < 4 || verificationCode.length < 4}>
                  ...
                  {authLoading ? t('portal.creating') : t('portal.createPinBtn')}
                </Button>
              </form>
            </CardContent>
          </Card>
          <button onClick={() => { setStep('lookup'); setPin(''); }} className="flex items-center gap-1 text-sm text-white/60 hover:text-white mx-auto mt-6">
            <ArrowLeft className="w-4 h-4" /> {t('portal.back')}
          </button>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen relative flex items-center justify-center p-6" style={{ backgroundImage: `url(${PORTAL_BG})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="relative max-w-md w-full">
          <div className="text-center text-white mb-8">
            ...
            <h1 className="text-2xl font-bold mb-1">{t('portal.createAccount')}</h1>
            <p className="text-sm text-white/70">{t('portal.noAccountFor')} {identifier}. {t('portal.createAccountSub')}</p>
          </div>
          <Card>
            <CardContent className="p-6">
              <form onSubmit={handleRegister} className="space-y-4">
                <div>
                  <Label htmlFor="rname">{t('portal.fullName')}</Label>
      ...
      <div className="min-h-screen relative flex items-center justify-center p-6" style={{ backgroundImage: `url(${PORTAL_BG})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="relative max-w-md w-full">
          <div className="text-center text-white mb-8">
            ...
            <h1 className="text-2xl font-bold mb-1">{t('portal.enterPin')}</h1>
            <p className="text-sm text-white/70">{t('portal.enterPinSub')}</p>
          </div>
          <Card>
            <CardContent className="p-6">
              <form onSubmit={handlePinSubmit} className="space-y-4">
                <div>
                  <Label htmlFor="pin">{t('portal.pin')}</Label>
                ...
                <Button type="submit" className="w-full" disabled={authLoading || !pin}>
                  ...
                  {authLoading ? t('portal.signingIn') : t('portal.signIn')}
                </Button>
              </form>
            </CardContent>
          </Card>
          <button onClick={() => { setStep('lookup'); setPin(''); }} className="flex items-center gap-1 text-sm text-white/60 hover:text-white mx-auto mt-6">
            <ArrowLeft className="w-4 h-4" /> {t('portal.useDifferent')}
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center p-6">
      <div className="max-w-md w-full">
        <div className="text-center text-white mb-8">
          ...
          <h1 className="text-2xl font-bold mb-1">{t('portal.title')}</h1>
          <p className="text-sm text-white/70">{t('portal.subtitle')}</p>
        </div>
        <Card>
          <CardContent className="p-6">
            <form onSubmit={handleLookup} className="space-y-4">
              <div>
                <Label htmlFor="lookup">{t('portal.phoneOrEmail')}</Label>
              ...
              <Button type="submit" className="w-full" disabled={lookupLoading || !identifier.trim()}>
                ...
                {lookupLoading ? t('portal.searching') : t('portal.continue')}
              </Button>
            </form>
